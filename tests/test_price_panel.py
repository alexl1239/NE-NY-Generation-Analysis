...
class TestPricePanelWebsite(unittest.TestCase):
    ...
    def test_dedicated_ownership_comparison_and_coverage_flags_are_visible(self):
        ...
        self.assertIn("const maximumPrice = 45", script)
        self.assertIn("for (const tick of [0, 15, 30, 45])", script)
        self.assertIn("The flag does not change the price", html)
        self.assertIn("Selected-sample prices by ownership", html)
